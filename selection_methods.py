...
def get_Top1(models, unlabeled_loader):
    ...
    with torch.no_grad():
        for inputs, _, _ in unlabeled_loader:
            ...
            prob = torch.sigmoid(scores, dim=1)
...
def get_same_pixels(models, unlabeled_loader):
    ...
    with torch.no_grad():
        for data  in unlabeled_loader:
            ...
            y_pred1, y_pred2  = models['backbone2'](inputs) 
            dis = torch.abs(y_pred1 - y_pred2) ###(8,2,256,256)
            dis_value = torch.sum(dis, axis=(1,2,3))
            # dis_value_tensor = torch.tensor(dis_value)
...
def get_max_entropy(models, unlabeled_loader):
    ...
    with torch.no_grad():
        for inputs, _ ,_ in unlabeled_loader:
            ...
            y_pred,_= models['backbone'](inputs) #(8,256,256,2)
            prob = torch.sigmoid(y_pred) #

            entropy_pixel = -(y_pred * torch.log(y_pred)).sum(dim=-1) #(16,256,256)
            entropy_pic = entropy_pixel.sum(dim = -1).sum(dim = -1)
            entropy = entropy_pic.detach().cpu()
...
def get_cumulative_entropy(models, unlabeled_loader, args=None):
    ...
    with torch.no_grad():
        for inputs, _, _ ,_ in unlabeled_loader:
            ...
            main_logit, features = models['backbone'](inputs)    

            _, pred_label = torch.max(main_logit, dim=3)
    ...
    # sub_prob = torch.sigmoid(sub_logit_all, dim=1)
    sub_prob = torch.sigmoid(sub_logit_all)
    # sub_prob = torch.sigmoid(sub_logit_all.unsqueeze(1)).squeeze(1)
    ...
    if args.query != "None":
        if args.query == 'AUM':
            ...
            sub_entropy = -(sub_prob * torch.log(sub_prob)).sum(dim=(1,2,3))  # Entropy
        ...
        with torch.no_grad():
            for inputs, _, _ in unlabeled_loader:
                ...
                for i in range(n_drop):
                    ...
                    prob = torch.sigmoid(scores, dim=1).unsqueeze(1)  # [B,        1, N_CLS]
...
def get_kcg(models, labeled_data_size, unlabeled_loader, args):
    ...
    with torch.no_grad():
        for inputs, _, _ in unlabeled_loader:
            with torch.cuda.device(CUDA_VISIBLE_DEVICES):
                inputs = inputs.cuda()
            _, features_batch= models['backbone'](inputs)
